...
const [collectedIds, setCollectedIds] = useState([]);
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(
          process.env.REACT_APP_API + "/predictions/user-bets",
          {
            credentials: "include",
        ...
        const betRes = await fetch(
          process.env.REACT_APP_API + `/leagues/matches/${activeTab}`,
          {
            credentials: "include",
            headers: { "x-auth-token": token },
          }
  ...
  const handlePlaceBet = async (matchId, option) => {
    try {
      const res = await fetch(
        process.env.REACT_APP_API + "/predictions/place-bet",
        {
          method: "POST",
          headers: {
  ...
  const isSelected = (matchId, option) => {
    return userOptions.some(
      (sel) => sel.matchId === matchId && sel.option === option
    );
  };
  ...
  const colectCash = async (id) => {
    try {
      const res = await fetch(
        process.env.REACT_APP_API + "/games-played/colectcash/" + id,
        {
          method: "POST",
          credentials: "include",
          headers: {
              ...
              <>
                <div key={group.league._id} className="league-block">
                  ...
                  {group.matches.map((bet, index) => (
                    ...
                    >
                      <div>
                        {new Date(bet.date).toLocaleTimeString("ro-RO", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </div>
                      ...
                      <div
                        className={`bet-option ${
                          isSelected(bet._id, "X") ? "selected" : ""
                        }`}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (bet.status === "scheduled")
                            handlePlaceBet(bet._id, "X");
                        }}
                      >
                      ...
                      <div
                        className={`bet-option ${
                          isSelected(bet._id, "2") ? "selected" : ""
                        }`}
                        onClick={(e) => {
                          e.stopPropagation();
                          if (bet.status === "scheduled")
                            handlePlaceBet(bet._id, "2");
                        }}
                      >
                      ...
                      <div className="responsive_trash">
                        {(() => {
                          const currentBet = userBet.find(
                            (ub) => ub.matchId._id === bet._id
                          ); // pariul userului pt acest meci
  ...
  className={`btn btn-success colect-button ${
    collectedIds.includes(currentBet._id) ? "collected" : ""
  }`}
  onClick={(e) => {
    e.stopPropagation();
    colectCash(currentBet._id);
  }}
  disabled={collectedIds.includes(currentBet._id)} // optional, să nu mai poată fi apăsat
>
  {currentBet.winCash} 💵
</button>
                              ...
                              <button
                                disabled
                                className="btn btn-warning colect-button"
                              >
                                {currentBet.winCash} 💵
                              </button>
                            ) : (
                            ...
                            <button
                              disabled
                              className="btn btn-danger colect-button-danger"
                            >
                              {currentBet.winCash} 💵
                            </button>
                          );
